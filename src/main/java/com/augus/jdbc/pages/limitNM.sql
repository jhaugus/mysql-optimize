...
DROP PROCEDURE IF EXISTS `p_update_t_user_batch`;

DELIMITER ;;
CREATE PROCEDURE `p_update_t_user_batch`()
BEGIN
    DECLARE i INT DEFAULT 1;
    WHILE i <= 1000000 DO
            -- 更新语句
...
-- 调用存储过程进行批量更新
CALL p_update_t_user_batch();

-- 提交事务（存储过程内部已经设定了 COMMIT，因此可以省略）
-- COMMIT;

-- 恢复默认配置
SET GLOBAL innodb_flush_log_at_trx_commit = 1; # 恢复默认刷盘方式
SET bulk_insert_buffer_size = 8 * 1024 * 1024; # 恢复默认8M
SET UNIQUE_CHECKS = ON; # 恢复唯一性校验
SET AUTOCOMMIT = ON; # 恢复自动提交





SELECT * FROM `t_user_batch` ORDER BY `add_time` LIMIT 10000;
SELECT * FROM `t_user_batch` ORDER BY `add_time` LIMIT 10000,10000;
SELECT * FROM `t_user_batch` ORDER BY `add_time` LIMIT 20000,10000;
SELECT * FROM `t_user_batch` ORDER BY `add_time` LIMIT 30000,10000; # 7秒？

SELECT * FROM `t_user_batch` ORDER BY `add_time` LIMIT 10000;
SELECT * FROM `t_user_batch` WHERE add_time > 10000 ORDER BY `add_time` LIMIT 10000;
SELECT * FROM `t_user_batch` WHERE add_time > 20000 ORDER BY `add_time` LIMIT 10000;
SELECT * FROM `t_user_batch` WHERE add_time > 30000 ORDER BY `add_time` LIMIT 10000;
